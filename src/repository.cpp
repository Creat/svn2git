...
public:
    ...
    public:
        ...
        void commitNote(const QByteArray &noteText, bool append, const QByteArray &commit = QByteArray());
    };
...
public:
    ...
    public:
        ...
        QIODevice *addFile(const QString &path, int mode, qint64 length)  { return txn->addFile(prefix + path, mode, length); }

        void commitNote(const QByteArray &noteText, bool append, const QByteArray &commit) { return txn->commitNote(noteText, append, commit); }
    };
    ...
    int createBranch(const QString &branch, int revnum, const QString &branchFrom, int revFrom) { return repo->createBranch(branch, revnum, branchFrom, revFrom); }

    int deleteBranch(const QString &branch, int revnum) { return repo->deleteBranch(branch, revnum); }
    ...
    void createAnnotatedTag(const QString &name, const QString &svnprefix, int revnum, const QByteArray &author, uint dt, const QByteArray &log) { repo->createAnnotatedTag(name, svnprefix, revnum, author, dt, log); }
    ...
    const QByteArray branchNote(const QString& branch) const { return repo->branchNote(branch); }
    void setBranchNote(const QString& branch, const QByteArray& noteText) { repo->setBranchNote(branch, noteText); }
...
int FastImportRepository::createBranch(const QString &branch, int revnum, const QString &branchFrom, int branchRevNum)
{
...
Repository::Transaction *FastImportRepository::newTransaction(const QString &branch, const QString &svnprefix, int revnum)
{
...
void FastImportRepository::createAnnotatedTag(const QString &ref, const QString &svnprefix, int revnum, const QByteArray &author, uint dt, const QByteArray &log)
{
    QString tagName = ref;
